...
func NewSwitchSocketFromConfig(ctx context.Context, other map[string]any) (api.Charger, error) {
	var cc struct {
		...
		Phases       int // optional, physically connected phases
	}
	...
	if cc.Phases < 0 || cc.Phases > 3 {
		return nil, fmt.Errorf("invalid phases: %d", cc.Phases)
	}
	...
	c.switchSocket.phases = cc.Phases
